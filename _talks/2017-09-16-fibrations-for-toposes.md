...
Borrowing from work of Ross Street we define a notion of (op)fibration in the 2-category $\Con$ of contexts developed in \citep{SK-AU}.  We establish that for every context extension (op)fibration $\thT_1 \to \thT_0$ in $\Con$ any model $M$ of $T_0$ in an elementary topos $\CS$ with nno\footnote{Natural number object} gives rise to an (op)fibration for toposes with codomain $\CS$, in the sense of Johnstone \citep{Fib-Par-Prod}.

[Fibrations For Toposes, PSSL 101, Leeds, UK ](http://www1.maths.leeds.ac.uk/~pmtng/Hazratpour.pdf)
