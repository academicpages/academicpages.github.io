...
Repeat n-1 times
  ...
  If no swaps
    quit
```
The number of comparison is $(n-1)^{2}$, so the running time is asymptotically $O(n^{2})$. In the best case, if all elements are already sorted, you compare only $(n-1)$ times, so it is $\Omega(n)$. We can't say anythong about theta notation for bubble sort because the upper and lower bounds are different but we seem to have done better asymptotically than selection sort. 
