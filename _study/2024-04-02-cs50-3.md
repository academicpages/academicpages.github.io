...
We can notice that string is an array of characters. 'string' is an abstraction in the CS50 library and it is an array of chars. But how does the computer know where the string - in this case 'HI!' - ends? When we've been using quote-unquote string and using get_string from the CS50 library, there's actually a special sentinel value at the end of every string in a computer's memory that tells the computer string stops here. The sentinel value is 1 byte of all 0. The implication is taht the computer using a loop and print out chars and whenever it reaches to the sentinel value, it stops. Thus, every string is N+1 bytes where n is the actual human length. We typically write the symbol of sentinel value as '\0' - it is an char-based representation of int 0 - in order to not confuse with '0'. The technic
