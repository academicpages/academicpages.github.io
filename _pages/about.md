---
permalink: /
title: "About me"
excerpt: "About me"
author_profile: true
redirect_from: 
  - /about/
  - /about.html
---

My name is Hadi Mohaghegh Dolatabadi (just call me Hadi! :D). Currently, I am a second year Ph.D. student at the University of Melbourne, working under the supervision of [Dr. Sarah Erfani](https://people.eng.unimelb.edu.au/smonazam/) and [Prof. Christopher Leckie](https://cis.unimelb.edu.au/people/staff.php?person_ID=6335). My main research interests are generative modeling and adversarial machine learning. Throughout the past year, I have been closely working on normalizing flows and their applications. Always happy to discuss research-related topics. 

__Research Interests__

* Normalizing Flows
* Generative Modeling
* Adversarial Robustness
* Machine Learning
* Signal Processing

News
======
 - Aug. 2021: Started as an applied scientist intern at Amazon!
 - Jun. 2021: I would be reviewing for INNF+, NeurIPS2021, and ICLR2021.
 - Dec. 2020: I am going to present AdvFlows at the [NeurIPS Pre-conference in Australia](http://vision-learning.xyz/neurips2020/).
 - Oct. 2020: AdvFlows blog post is [available online](https://hmdolatabadi.github.io/posts/2020/10/advflow/).
 - Sep. 2020: AdvFlows are accepted to NeurIPS2020!
 - Aug. 2020: Presented (virtually) my work at AISTATS2020.
 - Jul. 2020: Attended (virtually) Machine Learning Summer School (MLSS2020) at Max Planck Institute in Tuebingen.
 - Jun. 2020: A short version of my recent work was accepted to INNF+.
 - Jan. 2020: Linear Rational Spline flows got accepted to AISTATS2020!
 - Jun. 2019: Started my Ph.D. at the University of Melbourne.
