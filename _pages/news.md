...
- I am currently working on the potential applications of Kernelized Correlation Filters to hyperspectral aerial videos for object tracking. Additionally, I work on vehicle detection in the *Wide-Area-Motion-Imagery (WAMI)* platform by training a Deep Convolutional Neural Network on a *synthetic dataset* generated by the DIRSIG software. This way, one can detect vehicles in WAMI platform without using any WAMI training data. Some of the positive samples in synthetic (DIRSIG) and real dataset (WAMI) can be seen in the figure below. The trained Deep Learning model can classify the WAMI samples with `93%` accuracy.
...
- Our [Hyperspectral Aerial Video Set](https://buzkent86.github.io/datasets/) for vehicle tracking is uploaded. Please cite the paper below if you use this dataset for your research. Our dataset also contains the ground truth files of the vehicles in the scene. Below, you can find the grayscale version of the hyperspectral aerial video. Grayscale images are generated by summing the individual channels in visible wavelength range. You can download the MATLAB code with C-based detection module in this [link](https://github.com/buzkent86/CVPRW17_Paper_Code).
