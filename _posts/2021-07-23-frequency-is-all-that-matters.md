...
* We suppose there are three phases of heads focus on different representations. 
...
* When looking into the outsiders, we find they can be seperated into three types:
  ...
  * \[MASK] is born to fulfill the Masked LM task. Since it shares the same charactristics with periods and commas, it is also a drifter. In BERT's view, it is the randomly selected drifters (\[MASK]) that are responsible for the Masked LM task. 
...
Phase Two
======
* **Hypothsis: High constant-relation frequency makes attention from insiders the main concern for middle heads**
* Solid Results:
...
Phase Three
======
* **Hypothesis: Mased LM and high token frequency make attention from drifters the main concern for deep heads**
* Solid Results:
...
* Explanations:
  ...
  2. **To highlight attention from drifters**, other tokens need to avoid attending to the same tokens that drifters may attend to. Since drifters are motivated to attend broadly, they themselves become havens for those tokens to lie low. 
...
* We suggest that **what attention does not look at is all you need**: early heads attend to \[CLS], because they need information of other tokens; middle heads attend to \[SEP], because they need to highlight attention from insiders; deep heads attend to periods and commas, because they need to highlight attention from drifters (what they actually need is attention from \[MASK], but it is hard to differentiate it from all drifters). 
