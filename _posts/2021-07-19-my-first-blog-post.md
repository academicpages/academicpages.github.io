...
Phase Three
======
Naturally, deep heads will be significantly and directly impacted by the two unsupervised tasks: Masked LM and NSP. Not surprisingly, attention from \[CLS] is obviously broader than the average in last layer. Meanwhile, periods and commas draw over half of attentiion in the last two layers. We suppose that it is hard to differentiate \[MASK], periods, and commas. Masked LM will treat them the same, motivating mutual relations between the drifters and the other tokens. Possibly, the heads will learn finer-grained segment as a by-product, because it is convenient for tokens in the same part to attend to the traditional seperators (periods and commas) close to them. Moreover, this can greatly help the prediction of \[MASK]. Since periods and commas attract lots attention from other tokens, \[MASK] will only be able to build relations with tokens close to it which is exacly the most significant part for the prediction.
