...
<br><strong>B.</strong> Lo más complicado al construir la base de datos fue el año 2007, ya que el correspondiente Registro Estadístico de Defunciones Generales, compilado por el Instituto Nacional de Estadísticas y Censos (INEC), no incluyó en su anuario la categoría específica de "Eventos de intención no determinada". Tuve entonces que compilar los datos a partir de la base de datos bruta, disponible <a href="https://anda.inec.gob.ec/anda/index.php/catalog/190/study-description">[aquí]</a>. Con esa base de datos sin procesar, se puede construir dicha variable mediante los códigos proporcionados por la Clasificación Internacional de Enfermedades <a href="https://ais.paho.org/classifications/Chapters/">(CIE-10)</a>, que van desde Y10 hasta Y34.<br> 
</div>
<br>

<div style="display: flex; justify-content: center;">

<pre style="background-color: #f4f4f4; color: #333; padding: 20px; border: 2px solid #000; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-family: 'Courier New', Courier, monospace; line-height: 1.5; overflow-x: auto; font-size: 11px; max-width: 800px;">
<code>
####### 1. Chequeo formal de tendencias paralelas #######

# Filtrar datos para el periodo previo al tratamiento (2007-2009)
pre_treatment_data &lt;- subset(data, Year &lt;= 2009)

# Ajustar modelo con interacción entre TreatmentGroup y el año, 
# con efectos fijos de Provincia
model_pre_trend &lt;- lm(LnTasa ~ TreatmentGroup * factor(Year) 
                + as.factor(Provincia), data = pre_treatment_data)

# Calcular errores estándar robustos (HC1) agrupados por Provincia
robust_se_pre_trend &lt;- vcovHC(model_pre_trend, type = "HC1", 
                                        cluster = ~Provincia)
...
data &lt;- subset(data, !Provincia %in% c("StDomingo", "StElena",
                                            "Nacional"))

# Ajustar 'eventtime' como factor, tratando '0' por separado
data$eventtime &lt;- factor(data$eventtime, levels = c("0", 
unique(as.character(data$eventtime[data$eventtime != 0]))))

# Ajustar modelo con LnTasa, interacción entre TreatmentGroup y eventtime, 
# con efectos fijos de Provincia
model &lt;- lm(LnTasa ~ TreatmentGroup * eventtime 
                 + as.factor(Provincia), data = data)
...
model_placebo &lt;- lm(LnTasa ~ Placebo * factor(eventtime) 
                          + as.factor(Provincia), data = data)
robust_se_placebo &lt;- vcovHC(model_placebo, type = "HC1", 
                                 cluster = ~Provincia)
