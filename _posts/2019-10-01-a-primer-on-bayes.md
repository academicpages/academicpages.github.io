...
We toss a coin *N* times and use the sample mean of the random variable *X = 1{heads}* as an estimator of the probability of heads 𝜃. Is it a good estimator? As a frequentist, we might answer that question by computing its mean squared error (MSE) [appendix 1.1.1].

The sample mean is the optimal *unbiased* estimator in the sense that among all unbiased estimators, it has minimum variance —thus minimum MSE, see Rao-Blackwell theorem. The point is that the sample mean is a “good” estimator of 𝜃 because in a hypothetical infinite sequence of experiments just like the one we performed, it will typically give us a number that is close to 𝜃.

## 1.1.2 Interval estimation

Point estimation is an exercise in producing a single number that is in some sense a “best guess” at the value of a parameter 𝜃. Interval estimation aims to produce a range of values that are “plausible”.

What would an exact 1 - 𝛼 confidence interval look like for 𝜃 for *N* coin tosses? We use the CDF of the binomial distribution to build a *Clopper-Pearson* interval, see [appendix 1.1.2]. However this approach is too heavy in computation.

Instead, we consider intervals that have *asymptotic* 1 - 𝛼 coverage, using the ***central limit theorem*** of the general form:

$$\hat{\theta} \pm \Phi^{-1}(1-\alpha/2)\hat{SE}(\hat{\theta})\\where\;\Phi\;is\;the\;standard\;Gaussian\;CDF\;and\;SE\;the\;standard\;error\;of\;\hat{\theta}$$

## 1.1.3 Testing

We want to test wether the coin is fair through hypothesis H0 : 𝜃 = 1/2 and H1 : 𝜃 ≠ 1/2. The classical way is to use ***Neyman's fixed type I error rate testing***. We reject H0 when our estimate 𝜃_*hat* is far from 1/2. We use the Binomial distribution of 𝜃_*hat* to find *c* (according to our chosen type I error rate) **such as:
...
- Likelihood (also appearing in frequentist inference). It suggests that we are going to consider the success probability 𝜃 itself to be a random variable.

$$p(x_1,..., x_N|\theta)=\prod^N_{i=1}f_{\theta}(x_i)=\theta^{S_N}(1-\theta)^{N-S_N}$$

- Prior distribution. It encapsulates our prior beliefs about 𝜃 before observing data *(x1, ..., xN)*. There are a number of possibilities of how we can proceed.
...
For our coin tossing example, with the N Bernoulli likelihood, our prior is in the Beta family (see appendix [1.2.1]).
...
For our exemple, we use the CDF of the Beta distribution to compute the equal-tailed interval —see [appendix 1.2.3].

## 1.2.4 Hypothesis testing

We again need to be able to compute everything using p(𝜃 | **x**). Hypotheses are subsets of the parameter space – in this case the subset is just {𝜃 = 1/2}. 
...
where *f(𝜃)* is the density of a *Beta(a, b)* distribution and *q* is in [0, 1]. The left part is associated with *H0* and the right with *H1*. 
...
In [appendix 1.2.4] we compute our new posterior, and we compare it with frequentist p-values in the figure below.

![](Screenshot_2019-08-09_at_12-ffb1022c-876f-45fd-b321-34d65b5180f5.04.04.png)
...
where *I(𝜃)* is the Fisher information matrix. In [appendix 1.2.5] we show that this prior is actually the *Beta(1/2, 1/2)* prior*.*
...
Suppose we observe *N* iid data points *x1, ..., xN* from a *Poisson(𝜃)* distribution. It has a the form of a Gamma distribution, so we choose a prior with the same distribution family.

In [appendix 1.3.1], we show that the posterior has also the Gamma form, which is

$$Gamma(a,b)=\frac{b^a}{\Gamma(a)}\theta^{a-1}e^{-b\theta}$$

## 1.3.2 The Normal Distribution

The likelihood of the normal distribution looks like the kernel of a Gamma distribution, so we define the prior this way. It is in fact the *normal-inverse gamma* distribution. We claim this is conjugate, and we find the parameters of the posterior in [appendix 1.3.2].

## 1.3.3 Multinomial Distribution

The conjugate prior of the multinomial likelihood is a multivariate of the Beta distribution, and is called *Dirichlet* distribution. Details in [appendix 1.3.3].
...
See original paper for theorem of large number, convergence in probability (i.p.) and almost sure convergence (a. s.).
...
Always keep in mind that for frequentists, it is the statistic or estimator – in this case, *X_N* – that is random, and the parameter 𝜃 is some fixed, unknown number.
...
See original paper for central limit theorem and weak convergence.

Well, we know that *S_N* has the binomial distribution with parameters *N*, 𝜃. Its CDF can be expressed as:

$$F_{\theta}(k)=P(S\leq k)=\sum_{i=1}^k\binom{N}{j}\theta^j(1-\theta)^{N-j}$$

We construct a *Clopper-Pearson* interval *[a(S), b(S)]* with at least *(1 -* 𝛼*)* coverage by solving:

$$\sum_{i=S}^N\binom{N}{j}a^j(1-a)^{N-j}=\alpha/2\\\sum_{i=0}^S\binom{N}{j}b^j(1-b)^{N-j}=\alpha/2$$

Ok but heavy to compute. Instead : asymptotical approach using *central limit theorem*. **The interval take the general form:
...
Under the null, the sampling distribution of 𝜃_*hat* is given by

$$N\hat{\theta}\; \widetilde{~~~}\;Binomial(N, 1/2)$$

So to figure out *c* we compute the probability of rejection (which is no bigger than 𝛼)

$$\sum_{j=0}^{N(1/2-c)}\binom{N}{j}2^{-N}+\sum^{N}_{j=N(1/2+c)}\binom{N}{j}2^{-N}$$

solving for *c < 1/2* to make the quantity as large as possible while still being less than 𝛼 will give a test with at most 𝛼 type I error rate.
...
Integrating only the part involving 𝜃 we find *C(**x**)* and thus the posterior. See original paper for details.
...
**More generally**, if our null hypothesis is *H0 :* 𝜃 = *c,* our prior become
...
So the prior on 𝜎^{-2} is an *inverse Gamma distribution*

$$p(\sigma^2)\propto(\sigma^2)^{-a-1}e^{-\frac{b}{\sigma^2}}$$

Also, if 𝜎^{-2} were a constant, this would look like the kernel of a normal random variable with variance 𝜎^{-2}. The conjugate prior is:
