...
Neste post será apresentado como fazer teste de conexão TCP e também UDP. Com a atual crescente da adoção de Firewall pelas empresas, às vezes alguma aplicação pode não estar funcionando simplesmente pelo fato que ela não é "liberada" para funcionar na rede. 
Recentemente eu tive este problema, os testes através Linux eu sabia fazer com as ferramentas nc (Netcat) e traceroute,
já no Windows não tinha o conhecimento de como fazer, pois o telnet (que é possível habilitar no Windows), utiliza como padrão a comunicação via TCP.
