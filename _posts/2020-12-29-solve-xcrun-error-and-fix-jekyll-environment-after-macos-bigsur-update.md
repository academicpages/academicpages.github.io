...
When a new version of an operating system comes out, I usually don't install it right away unless it's absolutely necessary. So when Apple rolled out their latest macOs 11.1 BigSur OS back in November 2020, I decided to wait. Especially since the University of Arizona warned its employees that there might be some compatibility issues regarding [Sophos security software](https://www.sophos.com/) (which I use).

However, it is finally winter break and it was time to update to the newest OS. I knew I would have to fix several things on my development environment, so I **backed up my computer to an external drive** just in case I needed to revert to my previous OS for any reason. 
...
After updating to Big Sur, I wanted to add content to my website so I went through the usual steps:
...
However, running `sudo gem install json -v '2.3.1' --source 'https://rubygems.org/` also failed, prompting me to `install ruby-dev` or `ruby-devel`, so I tried to update brew first and see what happened:
...
Later, I found that I needed to make one small change to my `.gitignore` file. I needed to add [the full `/vendor/` directory](https://jekyllrb.com/tutorials/using-jekyll-with-bundler/) as opposed to just the `/vendor/bundle` which was sufficient in macOs Catalina:
...
- Inform myself.
  - Pay attention to what your everyday tool developers say about a new big release. In my case these are:
    - Python
    - Anaconda
    - Homebrew
    - Git
    - My antivirus, among others
...
- Make sure I have enough time to backup and solve any issues that might come up during or after the process.
- Backup my computer right before updating my OS (apart from my weekly backup).
...
Special thanks to [o9uz.dev](https://dev.to/o9uzdev) for [this post](https://dev.to/o9uzdev/macos-xcrun-error-invalid-active-developer-path-missing-xcrun-411a) in dev.to and to Ernest Ojeh for [this post](https://ernestojeh.com/fix-jekyll-on-macos-big-sur) pre-[2.60 homebrew](https://brew.sh/2020/12/01/homebrew-2.6.0/).
